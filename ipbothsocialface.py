...
while (cap.isOpened()):
    ...
    image = imutils.resize(image, width=1600, height=900)
    ...
    #cv2.namedWindow('FACE MASK AND SOCIAL DISTANCING', cv2.WINDOW_FREERATIO)
    #cv2.setWindowProperty('FACE MASK AND SOCIAL DISTANCING ', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
    hjk2 = cv2.resize(image, (1600, 840))
    cv2.imshow("FACE MASK AND SOCIAL DISTANCING", hjk2)
    # cv2.imshow("Face", x)
